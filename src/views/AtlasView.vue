<template>
  <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
    ...
    <div class="flex h-[calc(100vh-80px)]">
        ...
        <div class="flex-1 overflow-auto p-3">
          ...
          <div v-if="activeTool === 'umap'" class="h-full">
            <UMAPVisualization
              ...
              :loading="loading"
              :error="error"
              :title="`${formatMetadataLabel(selectedMetadataColumn)} UMAP`"
              :show-metadata-selector="true"
              :available-metadata-columns="availableMetadataColumns"
              v-model:selected-metadata-column="selectedMetadataColumn"
              :is-loading-metadata="isLoadingMetadata"
              :filtered-cell-indices="hasActiveFilters ? filteredCellIndices : null"
              :show-excluded-cells="true"
              :hide-filter-controls="true"
            />
          </div>

          <!-- Composition Analysis -->
          <div v-if="activeTool === 'composition'" class="h-full">
            <CompositionAnalysis
              :available-metadata-columns="availableMetadataColumns"
              :cell-metadata="compositionMetadata"
              :loading="loadingComposition"
              :filtered-cell-indices="hasActiveFilters ? filteredCellIndices : null"
              @fetch-metadata="fetchCompositionMetadata"
            />
          </div>
                ...
                >
                  ...
                  >
                    <div class="text-center">
                      <p class="text-xl mb-2"><AppIcon name="dna" size="xl" class="mx-auto mb-2" /></p>
        ...
        <input 
          ...
          type="text" 
              ...
              >
                ...
                >
                  <DownloadButtons
                    :disabled="!coexpressionData"
                    @download-png="downloadCoexpressionPNG"
                    @download-csv="downloadCoexpressionCSV"
            ...
            ></div>
                ...
                >
                  <div class="text-center text-red-600">
                    <p class="text-xl mb-2"><AppIcon name="warning" size="xl" class="mx-auto mb-2" /></p>
          ...
          type="text" 
              ...
              <div class="flex-1 flex flex-col min-h-0">
                ...
                >
                  ...
                  >
                    <div class="text-center">
                      <p class="text-xl mb-2"><AppIcon name="chart" size="xl" class="mx-auto mb-2" /></p>
                ...
                >
                  ...
                  <dl class="space-y-1 text-xs">
                    ...
                    <div class="flex justify-between">
                      <dt class="text-gray-600">Genes Used:</dt>
                      <dd class="font-medium text-gray-900">
                        {{ heatmapStats.genesUsed }} /
                        {{ heatmapStats.totalGenes }}
                      </dd>
                    </div>
                  </dl>
                  <div
                    v-if="
                      heatmapStats.genesNotFound &&
                      heatmapStats.genesNotFound.length > 0
                    "
                    class="mt-2 text-xs text-red-600"
                  >
                    Not found: {{ heatmapStats.genesNotFound.join(", ") }}
                  </div>
                </div>
      ...
      <!-- Cell Filter Panel (Slide-out) -->
      <CellFilterPanel
        v-model:is-open="isFilterPanelOpen"
        :total-cell-count="filterTotalCellCount"
        :filtered-cell-count="filteredCellCount"
        :active-filter-count="activeFilterCount"
        :has-active-filters="hasActiveFilters"
        :category-filters="categoryFilters"
        :available-columns="filterAvailableColumns"
        :column-values="filterColumnValues"
        :column-value-counts="filterColumnValueCounts"
        :gene-filters="geneFilters"
        :gene-expression-cache="filterGeneExpressionCache"
        :search-genes="searchGenes"
        v-model:global-logic="globalLogic"
        :preset-list="getPresetList"
        :encode-filters="encodeFiltersToUrl"
        @add-category-filter="addCategoryFilter"
        @remove-category-filter="removeCategoryFilter"
        @update-category-filter="updateCategoryFilter"
        @add-gene-filter="addGeneFilter"
        @remove-gene-filter="removeGeneFilter"
        @update-gene-filter="updateGeneFilter"
...
import { useAtlasMetadata } from "@/composables/useAtlasMetadata";
...
import { useCellFilter } from "@/composables/useCellFilter";
...
const atlasMetadata = useAtlasMetadata(datasetId);
const {
...
// Create a ref for filtered cell indices that will be populated by useCellFilter
const advancedFilteredCellIndices = ref(null);

const geneExpression = useGeneExpressionAnalysis(datasetId, umapData, selectedCellTypes, cellMetadata, advancedFilteredCellIndices);
const {
  geneInput,
  selectedGenes,
  selectedGene,
...
// Cell Filter composable - use atlas metadata columns
const cellFilter = useCellFilter(
  cellMetadata,
  fetchGeneExpression,
  datasetId,
  {
    externalColumns: availableMetadataColumns,
    externalColumnValues: categoricalMetadata,
    externalCellMetadata: compositionMetadata, // For cell counts per category value
  }
);
const {
  categoryFilters,
  geneFilters,
  isFilterPanelOpen,
  geneExpressionCache: filterGeneExpressionCache,
  globalLogic,
  availableColumns: filterAvailableColumns,
  columnValues: filterColumnValues,
  columnValueCounts: filterColumnValueCounts,
  totalCellCount: filterTotalCellCount,
  filteredCellCount,
  filteredCellIndices,
  filteredCellTypes,
  hasActiveFilters,
...
// Sync filtered cell indices to gene expression analysis
watch(filteredCellIndices, (newIndices) => {
  advancedFilteredCellIndices.value = hasActiveFilters.value ? newIndices : null;
}, { immediate: true });

watch(hasActiveFilters, (hasFilters) => {
  advancedFilteredCellIndices.value = hasFilters ? filteredCellIndices.value : null;
});

// Check URL for shared filter on mount
onMounted(async () => {
  const urlParams = new URLSearchParams(window.location.search);
  const filterParam = urlParams.get('filter');
  if (filterParam) {
    await decodeFiltersFromUrl(filterParam);
  }
});

// Watch for metadata column changes to reload UMAP data
watch(selectedMetadataColumn, async (newColumn, oldColumn) => {
  if (newColumn !== oldColumn) {
    await fetchUMAPData(newColumn);
  }
});
...
// coexpressionChart is now from useGeneExpressionAnalysis composable

// API Functions
const fetchDatasetInfo = async () => {
  try {
    isLoadingMetadata.value = true;
    const h5adData = await fetchDatasetInfoComposable(
      (categoricalMetadataData) => {
        // Atlas-specific: Initialize categorical metadata
        initializeCategoricalMetadata(categoricalMetadataData);
      }
    );
...
const fetchUMAPData = async (metadataColumn = null) => {
  try {
    loading.value = true;
    const metaCol =
      metadataColumn || selectedMetadataColumn.value || "cell_type";
    const response = await fetch(
      `${API_BASE_URL}/h5ad/${datasetId.value}/plot-data?embedding=umap&metadata=${metaCol}`
    );
...
// Watch for metadata column changes (Atlas-specific)
watch(selectedMetadataColumn, async (newColumn, oldColumn) => {
  if (newColumn !== oldColumn) {
    await fetchUMAPData(newColumn);
  }
...
// fetchCompositionMetadata and fetchMetadataColumnValues are now from useAtlasMetadata composable
// The watch for availableMetadataColumns is also in the composable

// formatMetadataLabel is now from useAtlasMetadata composable
...
// For AtlasView, we don't need the metadata enrichment since dataset selector is hidden
const fetchAvailableDatasets = async () => {
  return await fetchAvailableDatasetsComposable();
};

// Format dataset label with metadata
// parseGeneInput and saveGeneSetToCart are now from useGeneExpressionAnalysis composable

// getBivariateColor, hexToRgb, rgbToHex are now from useColorPalettes composable
...
// Co-expression and gene expression functions are now from useGeneExpressionAnalysis composable
...
// Download functions for co-expression
const downloadCoexpressionPNG = () => {
  if (!coexpressionChart.value) return;

  try {
    const chartInstance = coexpressionChart.value;
    const echartsInstance =
      chartInstance.getEchartsInstance?.() || chartInstance.chart;
    if (echartsInstance) {
      ...
      const filename = `coexpression_${coexpressionSettings.value.gene1}_${
    ...
    console.warn("Could not download co-expression plot:", err);
    alert("Download functionality not available");
  }
};

const downloadCoexpressionCSV = () => {
  if (!coexpressionData.value || !coexpressionData.value.scatterData) {
  ...
  try {
    const rows = [];
    const { scatterData, gene1, gene2 } = coexpressionData.value;
    const xAxisName = coexpressionSettings.value.xAxis || "UMAP1";
    const yAxisName = coexpressionSettings.value.yAxis || "UMAP2";
    ...
    scatterData.forEach((point) => {
      const [x, y] = point.value || [];
      const expr1 = point.expr1 || 0;
      const expr2 = point.expr2 || 0;
      rows.push(
        [
          x?.toFixed(6) || "",
          y?.toFixed(6) || "",
    ...
    const link = document.createElement("a");
    const filename = `coexpression_${gene1}_${gene2}_${Date.now()}.csv`;
    link.download = filename;
...
// Heatmap statistics computed property
const heatmapStats = computed(() => {
  if (!heatmapData.value || !heatmapData.value.data) return null;

  const genes = heatmapData.value.genes || [];
  const requestedGenes = parseGeneInput(heatmapSettings.value.geneList);

  // Determine genes used vs not found
  const genesUsed = genes.length;
  const totalGenes = requestedGenes.length;
  const genesNotFound = requestedGenes.filter(
    (gene) => !genes.some((g) => g.toUpperCase() === gene.toUpperCase())
  );
